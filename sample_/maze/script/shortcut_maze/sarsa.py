...
import sample_.maze.helper as Helper
from sample_.maze.task import MazeTask
from sample_.maze.callback import MazePerformanceWatcher, MazeTransformer

from kyoka.algorithm_.sarsa import Sarsa, SarsaTabularActionValueFunction
from kyoka.policy_ import EpsilonGreedyPolicy

class MazeTabularActionValueFunction(SarsaTabularActionValueFunction):
...
TEST_LENGTH = 200
MAZE_FILE_PATH = os.path.join(os.path.dirname(__file__), "shortcut.txt")
TRANSFORMATION_FILE_PATH = os.path.join(os.path.dirname(__file__), "shortcut_transformed.txt")
